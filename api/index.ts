...
// ── Schema (inline to avoid path alias issues in serverless) ────────────────
...
const surfSpots = pgTable("surf_spots", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
...
const insertSurfSpotSchema = createInsertSchema(surfSpots).omit({
  id: true,
  createdAt: true,
});
...
const surfSessions = pgTable("surf_sessions", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
...
const insertSurfSessionSchema = createInsertSchema(surfSessions).omit({
  id: true,
  createdAt: true,
});
...
const db = drizzle(pool, {
  schema: { users, surfSpots, conversations, messages, surfSessions },
});
...
const storage = {
  ...
  async createSpot(
    spot: z.infer<typeof insertSurfSpotSchema>
  ) {
  ...
  async getAllSessions() {
    return db
      .select()
      .from(surfSessions)
      .orderBy(desc(surfSessions.sessionDate));
  },
  async getSession(id: string) {
    const [s] = await db
      .select()
      .from(surfSessions)
      .where(eq(surfSessions.id, id));
    return s;
  },
  async createSession(
    s: z.infer<typeof insertSurfSessionSchema>
  ) {
    const [created] = await db.insert(surfSessions).values(s).returning();
    return created;
  },
...
// ── Caches (per-invocation in serverless, but still helps within warm instances)
...
// ── Helpers (weather, geocode, etc.) ────────────────────────────────────────

function generateSyntheticWindData(
  s: number,
  n: number,
  w: number,
  e: number
  ...
  for (let lat = s + latStep / 2; lat <= n; lat += latStep) {
    for (let lng = w + lngStep / 2; lng <= e; lng += lngStep) {
      ...
      const variation =
        Math.sin(lng * 0.05 + t) * 8 + Math.cos(lat * 0.08 + t * 0.7) * 5;
      const dirVariation =
        Math.sin(lat * 0.1 + lng * 0.05 + t * 0.3) * 30;
      ...
      const waveHeight = isCoastal
        ? Math.max(
            0.2,
            windSpeed * 0.08 + Math.sin(lat * 0.15 + t) * 0.5
          )
        : null;
      const waveDir = isCoastal
        ? (windDir + 10 + Math.sin(lng * 0.1) * 15) % 360
        : null;
      const wavePeriod = isCoastal
        ? Math.max(4, 6 + windSpeed * 0.15 + Math.sin(lat * 0.2) * 2)
        : null;
      ...
      points.push({
        ...
        temp:
          Math.round(
            (25 - Math.abs(lat) * 0.4 + Math.sin(lng * 0.05) * 3) * 10
          ) / 10,
...
async function fetchForecastForAI(
  lat: number,
  lng: number
): Promise<string> {
  try {
    ...
    if (weather.current) {
      ...
      current.push(
        `Wind: ${weather.current.wind_speed_10m} km/h from ${weather.current.wind_direction_10m}°`
      );
      if (weather.current.wind_gusts_10m)
        current.push(`Wind Gusts: ${weather.current.wind_gusts_10m} km/h`);
    }
    if (marine.current) {
      current.push(
        `Wave Height: ${marine.current.wave_height}m (${(marine.current.wave_height * 3.28).toFixed(1)}ft)`
      );
    ...
    if (hourly && marineHourly) {
      for (
        let i = 0;
        i < Math.min(72, hourly.time?.length || 0);
        i += 3
      ) {
        ...
        forecastLines.push(
          `${t}: Waves ${wh}m, Period ${wp}s, Swell ${sw}m@${swDir}°, Wind ${ws}(gusts ${wg})km/h@${wd}°, ${temp}°C`
        );
      }
    }
...
const KNOWN_SURF_SPOTS: Record<string, KnownSpot> = {
  pipeline: { name: "Pipeline, North Shore, Oahu", lat: 21.665, lng: -158.053 },
  backdoor: { name: "Backdoor Pipeline, Oahu", lat: 21.665, lng: -158.053 },
  "sunset beach": { name: "Sunset Beach, Oahu", lat: 21.678, lng: -158.042 },
  waimea: { name: "Waimea Bay, Oahu", lat: 21.6419, lng: -158.0656 },
  "north shore": { name: "North Shore, Oahu", lat: 21.58, lng: -158.104 },
  ...
  keramas: { name: "Keramas, Bali", lat: -8.5875, lng: 115.455 },
  ...
  hossegor: { name: "Hossegor, France", lat: 43.667, lng: -1.398 },
  ...
  "puerto escondido": { name: "Puerto Escondido, Mexico", lat: 15.861, lng: -97.073 },
  raglan: { name: "Raglan, New Zealand", lat: -37.8047, lng: 174.8619 },
  "g-land": { name: "G-Land, Java", lat: -8.735, lng: 114.37 },
  "desert point": { name: "Desert Point, Lombok", lat: -8.749, lng: 115.825 },
  "margaret river": { name: "Margaret River, Australia", lat: -33.953, lng: 114.9963 },
  "gold coast": { name: "Gold Coast, Australia", lat: -28.1693, lng: 153.5517 },
  "byron bay": { name: "Byron Bay, Australia", lat: -28.6428, lng: 153.612 },
  noosa: { name: "Noosa, Australia", lat: -26.3814, lng: 153.0889 },
  bali: { name: "Bali, Indonesia", lat: -8.8294, lng: 115.0851 },
  hawaii: { name: "North Shore, Oahu, Hawaii", lat: 21.58, lng: -158.104 },
  oahu: { name: "North Shore, Oahu", lat: 21.58, lng: -158.104 },
  maui: { name: "Hookipa, Maui", lat: 20.936, lng: -156.356 },
  ...
  france: { name: "Hossegor, France", lat: 43.667, lng: -1.398 },
  morocco: { name: "Taghazout, Morocco", lat: 30.5451, lng: -9.711 },
  "costa rica": { name: "Playa Hermosa, Costa Rica", lat: 9.5578, lng: -84.5815 },
  nicaragua: { name: "Popoyo, Nicaragua", lat: 11.4675, lng: -86.063 },
  ...
  mexico: { name: "Puerto Escondido, Mexico", lat: 15.861, lng: -97.073 },
  fiji: { name: "Cloudbreak, Fiji", lat: -17.8692, lng: 177.1881 },
  tahiti: { name: "Teahupoo, Tahiti", lat: -17.8539, lng: -149.2556 },
  maldives: { name: "North Male Atoll, Maldives", lat: 4.255, lng: 73.453 },
  mentawai: { name: "Mentawai, Indonesia", lat: -2.0861, lng: 99.6078 },
  taghazout: { name: "Taghazout, Morocco", lat: 30.5451, lng: -9.711 },
  sayulita: { name: "Sayulita, Mexico", lat: 20.868, lng: -105.439 },
  "santa cruz": { name: "Santa Cruz, California", lat: 36.9514, lng: -122.0263 },
  "la jolla": { name: "La Jolla, California", lat: 32.8328, lng: -117.2713 },
  "skeleton bay": { name: "Skeleton Bay, Namibia", lat: -24.764, lng: 14.526 },
...
function extractMentionedSpots(message: string): KnownSpot[] {
  ...
  const sortedKeys = Object.keys(KNOWN_SURF_SPOTS).sort(
...
app.use(
  session({
    ...
    secret: process.env.SESSION_SECRET || "surfcast-secret-key",
    resave: false,
...
app.get("/api/forecast/:lat/:lng", async (req, res) => {
  try {
    ...
    if (forecastCache.size > 200) {
      const oldest = Array.from(forecastCache.entries()).sort(
...
app.get("/api/grid-weather", async (req, res) => {
  try {
    ...
    if (gridWeatherCache.size > 100) {
      const oldest = Array.from(gridWeatherCache.entries()).sort(
        (a, b) => a[1].timestamp - b[1].timestamp
      );
  ...
  try {
    ...
    const fetchBeachCams = async (
      ...
      const extraPages = Math.min(
        3,
        Math.ceil(Math.min(total, 200) / PAGE_SIZE) - 1
      );
      if (extraPages > 0) {
    ...
    const results = await Promise.all(
      points.map((p) =>
        fetchBeachCams(
          p.lat,
          p.lng,
          totalCells <= 6 ? QUERY_RADIUS : effectiveRadius
        )
      )
    );

    const seen = new Set<number>();
    const webcams = results
      .flat()
      .filter((cam: any) => {
      ...
      .map((cam: any) => ({
        ...
        player:
          cam.player?.lifetime?.day ||
          cam.player?.lifetime?.month ||
          null,
      }));
...
app.post("/api/sessions", async (req, res) => {
  try {
    ...
    const s = await storage.createSession(parsed.data);
...
app.post("/api/chat", async (req, res) => {
  try {
    const {
      ...
      forecastData: clientForecastData,
    } = req.body;
      ...
      try {
        ...
        if (w && m) {
          ...
          const idx = w.time
            ? Math.max(
                0,
                w.time.findIndex((t: string) => new Date(t) >= now) - 1
              )
            : 0;
          ...
          if (w.time && w.time.length > ci + 6) {
            ...
            for (let h = 1; h <= 6; h++) {
              ...
              if (fi < w.time.length) {
                lines.push(
                  `    ${new Date(w.time[fi]).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}: Wind ${w.wind_speed_10m?.[fi]?.toFixed(0)} mph, Waves ${m.wave_height?.[fi]?.toFixed(1)} ft, Period ${m.wave_period?.[fi]?.toFixed(0)}s`
                );
              }
            }
          }
    ...
    const savedSpotMatch = spots.find((s) =>
      message.toLowerCase().includes(s.name.toLowerCase())
    );
    if (savedSpotMatch) {
      const data = await fetchForecastForAI(
        savedSpotMatch.latitude,
        savedSpotMatch.longitude
      );
      if (data)
    ...
    if (savedSpotMatch) {
      usedCoords.add(
        `${savedSpotMatch.latitude.toFixed(1)},${savedSpotMatch.longitude.toFixed(1)}`
      );
    ...
    locationForecasts.push(
      ...spotResults.filter((r): r is string => r !== null)
    );

    if (mentionedSpots.length === 0 && !savedSpotMatch) {
      const cleaned = message
        .replace(
          /how|are|is|the|conditions|in|at|right|now|today|tomorrow|what|about|like|forecast|for|surf|waves|surfing|good|bad|check/gi,
          ""
        )
        .trim();
      if (cleaned.length > 2) {
    ...
    if (locationForecasts.length === 0 && !forecastContext) {
      ...
      const isGeneralSurfQ = generalTerms.some((t) =>
